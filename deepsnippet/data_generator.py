...
#
#         word = AudioDataGenerator.to_word(path)
#
#         label[AudioDataGenerator.to_label_index(word)] = 1
#
#         return label
#
#     @staticmethod
...
class MinibatchGenerator(object):
    def __init__(self, data: list, target: np.ndarray, batch_size, max_len=None,
        ...
        self._indices = range(self.data_size)

        self.steps_per_epoch = self.data_size // self.minibatch_size
        self.actual_used_size = self.steps_per_epoch * self.minibatch_size
...
#     def get_batch(self):
#
#         if self.cur_index + self.minibatch_size > self.data_size:
#             current_paths = self.data_paths[self.cur_index:]
#
#             self.data_paths = shuffle(self.data_paths)
#
#             next_cur_index = self.cur_index + self.minibatch_size - self.data_size
#             current_paths.extend(self.data_paths[:next_cur_index])
#
#             features = self.featurize(current_paths, self.minibatch_size)
...
#                               mfcc_params):
#         self.train_data_gen = AudioDataGenerator(train_data_paths, minibatch_size=self.minibatch_size,
...
#                                                  input_feature=self.input_feature)  # type: AudioDataGenerator
#
#         self.valid_data_gen = AudioDataGenerator(valid_data_paths, minibatch_size=self.minibatch_size,
#                                                  fft_length=self.fft_length, sample_rate=self.sample_rate,
